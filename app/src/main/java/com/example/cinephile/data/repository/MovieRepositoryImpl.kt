...
class MovieRepositoryImpl @Inject constructor(
    ...
    override suspend fun rateMovie(movieId: Long, rating: Float): MovieEntity? {
        return withContext(Dispatchers.IO) {
            try {
                Log.d(TAG, "rateMovie: Setting rating $rating for movie $movieId")
                movieDao.updateRating(movieId, rating)
                recommendationDao.clear()
                val updated = movieDao.getById(movieId)
                Log.d(TAG, "rateMovie: Successfully updated rating. Movie exists: ${updated != null}, new rating: ${updated?.userRating}")
                updated
            } catch (e: Exception) {
                Log.e(TAG, "rateMovie: Error updating rating for movie $movieId", e)
                null
            }
        }
    }
